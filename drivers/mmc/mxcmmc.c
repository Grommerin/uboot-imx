...
{
        printf("MY INSERT: start mxcmci_init()\n");
        
	struct mxcmci_host *host = mmc->priv;
        printf("MY INSERT: prepend mxcmci_softreset(host)\n");
	mxcmci_softreset(host);
        printf("MY INSERT: prepend host->rev_no = readw(&host->base->rev_no))\n");
	...
	writel(0, &host->base->int_cntr);
        printf("MY INSERT: mxcmci_init() return 0\n");
	return 0;
}
